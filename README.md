...
By default, replay now follows each segment's `timestamp_ms` timing from the transcript file (realtime pacing). Use `--speed` to accelerate for demos:
...
- **0s:** Transcript segments start streaming in at transcript pace.
...
│   ├── api/                 # FastAPI Backend
│   │   ├── app/
│   │   │   ├── models/      # SQLAlchemy Models (Event Sourcing)
│   │   │   ├── routers/     # WebSocket & REST Endpoints
│   │   │   └── services/    # Business Logic (Rules, LLM, PII)
│   │   └── tests/           # Pytest Suite
│   └── web/                 # Next.js Frontend
│       ├── src/
│       │   ├── components/  # UI Components (GuidancePanel, Transcript)
│       │   └── stores/      # Zustand State Management
├── infra/
│   ├── scripts/             # Replay Tools & Seed Scripts
│   └── transcripts/         # Sample Call Data (JSON)
